...
uint32_t index_alpha(const argon2_instance_t *instance, const argon2_position_t *position, uint32_t pseudo_rand,
                     int same_lane) {
  /*
   * Pass 0:
...
int validate_inputs(const argon2_context *context) {
  ...
  if (ARGON2_MAX_MEMORY < UINT32_MAX) {
    if ((uint32_t) ARGON2_MAX_MEMORY < context->m_cost) {
