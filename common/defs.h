...
struct session_key {
  struct in6_addr address __attribute__((aligned(16))); // 客户端地址
  __be16          sport;                                // 客户端源端口
...
struct user_info {
  struct in6_addr address __attribute__((aligned(16))); // IP address in network byte order
...
struct egress_peer_key {
  struct in6_addr address __attribute__((aligned(16))); // Server address in network byte order
...
// Config struct
struct config {
  __u64 packets_processed __attribute__((aligned(8)));
